...
def parse_fit_file(
    ...
    athlete_id: int,
    file_name: str,
) -> tuple[schemas.ActivityBase, list[dict], list[dict], list[schemas.PotentialPerformanceMarkerCreate]]:
    """
    ...
    record_fields = ["timestamp", "power", "heart_rate", "cadence", "speed", "altitude", "position_lat", "position_long"]

    for message in fitfile.get_messages():
        if message.name == 'lap':
            lap_data = {}
            lap_number = message.get_value('message_index')
            lap_data['lap_number'] = lap_number if lap_number is not None else len(lap_dicts) + 1
    ...
    if device_info_msg:
        ...
        if manufacturer and isinstance(manufacturer, str) and product_name and isinstance(product_name, str):
            device = crud.find_or_create_device(db, athlete_id, brand=manufacturer, model=product_name)
    ...
    # --- TRIMP Calculation ---
    trimp = 0
    hr_data = [r.get("heart_rate") for r in record_dicts if r.get("heart_rate") is not None]
    lthr_record = crud.get_latest_lthr(db, athlete_id=athlete_id, activity_date=start_time)
    ...
    ftp_record = crud.get_latest_ftp(db, athlete_id=athlete_id, activity_date=start_time)
    ...
    if power_data:
        ...
        if best_20_min:
            ...
            if hr_data:
                segment_hr = hr_data[best_20_min['start_index']:best_20_min['end_index'] + 1]
