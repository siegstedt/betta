...
def upsert_daily_metric(db: Session, athlete_id: int, date: date, ctl: float, atl: float, tsb: float, tss: int, if_avg: float):
    """Creates or updates a DailyPerformanceMetric entry for a specific date."""
    db_metric = db.query(models.DailyPerformanceMetric).filter_by(athlete_id=athlete_id, date=date).first()
    ...
    if db_metric:
        ...
        db_metric = models.DailyPerformanceMetric(
            athlete_id=athlete_id, date=date, ctl=ctl, atl=atl, tsb=tsb, tss=tss, if_avg=if_avg
        )
        db.add(db_metric)
...
def get_latest_weight(db: Session, athlete_id: int, activity_date: Optional[datetime] = None):
    """
    ...
    return db.query(models.AthleteMetric.value)\
...
def get_latest_lthr(db: Session, athlete_id: int, activity_date: Optional[datetime] = None):
    """
...
def get_latest_ftp(db: Session, athlete_id: int, activity_date: Optional[datetime] = None):
    """
...
def get_metric_history(db: Session, athlete_id: int, metric_type: models.MetricType):
    ...
    return db.query(models.AthleteMetric)\
        .filter(models.AthleteMetric.athlete_id == athlete_id)\
...
def get_power_records_for_date_range(db: Session, athlete_id: int, start_date: date, end_date: date):
    """Fetches all power data points for an athlete within a given date range."""
    return db.query(models.ActivityRecord.power)\
...
def get_daily_aggregates_for_metric(db: Session, athlete_id: int, start_date: date, end_date: date, metric_column: str, agg_func):
    """
    ...
    return db.query(
        ...
        func.date(models.Activity.start_time)
    ).order_by(
        func.date(models.Activity.start_time)
    ).all()
...
def get_pending_markers(db: Session, athlete_id: int):
    ...
    return db.query(models.PotentialPerformanceMarker).filter_by(
        athlete_id=athlete_id,
        status=models.PotentialMarkerStatus.PENDING
    ).order_by(desc(models.PotentialPerformanceMarker.date_detected)).all()
