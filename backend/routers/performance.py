...
def get_weight_at_date(athlete_id: int, date: datetime, db: Session = Depends(get_db)):
    ...
    """
    weight_record = crud.get_latest_weight(
        db, athlete_id=athlete_id, activity_date=date
    )
    if weight_record:
...
def get_ftp_at_date(
    athlete_id: int, date: Optional[datetime] = None, db: Session = Depends(get_db)
):
    """
...
def get_pmc_data_endpoint(
    ...
    """
    last_metric_before_range = crud.get_latest_daily_metric_before_date(
        db, athlete_id, start_date
    )
...
def get_mmp_curve_endpoint(
    ...
    power_records = crud.get_power_records_for_date_range(
...
def get_weekly_workload_endpoint(
    ...
    if not metric_column:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid metric type for workload analysis: {metric}",
        )
    ...
    return services.activity_processing.process_weekly_workload(
        daily_aggregates, activity_date
    )


@router.get(
    "/potential-markers", response_model=List[schemas.PotentialPerformanceMarker]
)
...
@router.put(
    "/potential-markers/{marker_id}", response_model=schemas.PotentialPerformanceMarker
)
def action_on_potential_marker(
    athlete_id: int,
    marker_id: int,
    ...
    marker = (
        ...
        .filter(models.PotentialPerformanceMarker.id == marker_id)
        .first()
    )
    if not marker:
    ...
    if marker.athlete_id != athlete_id:
        raise HTTPException(
            status_code=403, detail="Marker does not belong to this athlete"
        )
    ...
    if marker_action.action == "accept":
        ...
        raise HTTPException(
            status_code=400, detail="Invalid action. Must be 'accept' or 'dismiss'."
        )
