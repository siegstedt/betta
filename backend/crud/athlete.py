...
def get_athlete(db: Session, athlete_id: int):
    return db.query(models.Athlete)\
        .options(joinedload(models.Athlete.metrics), joinedload(models.Athlete.activities))\
        .filter(models.Athlete.athlete_id == athlete_id).first()

def update_athlete(db: Session, athlete_id: int, athlete_data: schemas.AthleteUpdate):
    db_athlete = db.query(models.Athlete).filter(models.Athlete.athlete_id == athlete_id).first()
    if not db_athlete:
        return None
