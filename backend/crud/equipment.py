...
def find_or_create_device(db: Session, athlete_id: int, brand: str, model: str) -> models.Equipment:
    """Finds an existing device or creates a new one if it doesn't exist."""
    existing_device = get_equipment_by_brand_model_type(db, athlete_id, brand, model, models.EquipmentType.DEVICE)
    if existing_device:
...
def get_equipment_by_brand_model_type(db: Session, athlete_id: int, brand: str, model: str, eq_type: models.EquipmentType):
    """Finds a piece of equipment by its brand, model, and type for a specific athlete."""
    return db.query(models.Equipment).filter_by(
        athlete_id=athlete_id, brand=brand, model=model, equipment_type=eq_type
    ).first()

def update_equipment(db: Session, equipment_id: int, equipment_data: schemas.EquipmentUpdate):
