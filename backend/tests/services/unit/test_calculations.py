...
def test_calculate_normalized_power_short_activity_fallback_to_average():
    ...
    assert calculate_normalized_power(power_data) == 120 # (100+110+120+130+140)/5 = 120
...
def test_estimate_power_tacx_negative_speed():
    assert estimate_power_tacx(speed_kmh=-10, setting=5) == -10 * (2.5 + 4 * 1.4815) # Should return negative power based on formula
...
def test_calculate_time_in_zones_zero_threshold():
    ...
    threshold = 0
    expected_zones = {zone_name: 0 for zone_name in POWER_ZONE_DEFINITIONS}
    assert calculate_time_in_zones(data_stream, threshold, POWER_ZONE_DEFINITIONS) == expected_zones
...
def test_calculate_historical_mmp_valid_data():
    ...
    expected = [
        ...
        {"duration": 10, "power": 155}, # (110+120+130+140+150+160+170+180+190+200)/10 = 155
    ]
    assert calculate_historical_mmp(power_data, intervals) == expected
...
def test_recalculate_pmc_from_date_no_previous_metrics(mock_crud):
    ...
    with patch('backend.services.calculations.date') as mock_date:
        ...
        mock_date.side_effect = lambda *args, **kw: date(*args, **kw) # Allow date(Y,M,D) calls
...
def test_find_best_n_minute_average_short_data():
    ...
    assert find_best_n_minute_average(data_stream, 1) == {} # Interval 1 min (60s) is too long
...
def test_find_best_n_minute_average_multiple_peaks():
    data_stream = [10] * 60 + [30] * 60 + [20] * 60 + [35] * 60 # 1 min at 10, 1 min at 30, 1 min at 20, 1 min at 35
    interval_minutes = 1
    result = find_best_n_minute_average(data_stream, interval_minutes)
