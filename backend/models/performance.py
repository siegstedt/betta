...
class AthleteMetric(Base):
    ...
    metric_type = Column(
        SQLAlchemyEnum(MetricType, name="metric_type_enum"), nullable=False
    )
...
class PotentialPerformanceMarker(Base):
    ...
    metric_type = Column(
        SQLAlchemyEnum(MetricType, name="potential_metric_type_enum"), nullable=False
    )
    value = Column(Float, nullable=False)
    date_detected = Column(DateTime, nullable=False)
    status = Column(
        SQLAlchemyEnum(PotentialMarkerStatus, name="potential_marker_status_enum"),
        nullable=False,
        default=PotentialMarkerStatus.PENDING,
    )
